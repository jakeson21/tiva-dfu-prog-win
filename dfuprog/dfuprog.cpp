...
ParseCommandLine(int argc, char *argv[])
{
    ...
    if(g_bEnumOnly)
    {
        ...
        if(g_iDeviceIndex || g_ulLength || g_ulAddress || g_bBinary ||
           !g_pszFile.empty() || g_bUpload || g_bClear || g_bSwitchMode)
        {
            //
...
//*****************************************************************************
int 
InitiateDFUMode(size_t inBaudRate, const std::string& inPort)
{
    std::wstring portname(L"\\\\.\\");
    portname += std::wstring(inPort.begin(), inPort.end());
    ...
    if (hComm == INVALID_HANDLE_VALUE) {
        ...
        std::cout << "\nCannot open Communication Port " << inPort.c_str() << std::endl;
    ...
    if ((m_bPortReady = GetCommState(hComm, &m_dcb)) == 0) {
        QUIETPRINT("GetCommState Error");
        CloseHandle(hComm);
        return false;
    }
    m_dcb.BaudRate = inBaudRate;
    m_dcb.ByteSize = 8;
    m_dcb.Parity = 0;
    m_dcb.StopBits = 1;
    m_dcb.fBinary = TRUE;
...
UploadImage(tLMDFUHandle hHandle, tLMDFUParams *psDFU)
{
    ...
    if(eRetcode == DFU_OK)
    {
        ...
        if(!g_bOverwrite)
        {
            eErr = fopen_s(&fh, g_pszFile.c_str(), "rb");
            if(eErr == 0)
            {
                //
                ...
                else
                {
                    ...
                    // file.
                    //
                    printf("File \"%s\" exists. Overwrite? (Y/N): ", g_pszFile.c_str());
        ...
        //
        eErr = fopen_s(&fh, g_pszFile.c_str(), "wb");
...
DownloadImage(tLMDFUHandle hHandle, tLMDFUParams *psDFU)
{
    ...
    else
    {
        ...
        eRetcode = _LMDFUDownloadBin(hHandle, pcFileBuf, (unsigned long)iLen,
                                    g_ulAddress, g_bSkipVerify, handle);
    }
...
main(int argc,char* argv[])
{
    ...
    {
        std::cout << "Skipping COM port connection - Assumes already in DFU mode" << std::endl;
    }
