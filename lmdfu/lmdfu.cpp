...
//             with the TI Tiva USB boot loader.
//
...
// Stellaris and Tiva part numbers.
//
//*****************************************************************************
typedef struct
{
...
#define NUM_TIVA_PARTS (sizeof(g_sPartNumMap) / sizeof(tPartNumMapping))
...
//****************************************************************************
//
// Query the DFU device information structure (for Tiva-compatible
// devices).
//
//****************************************************************************
static tLMDFUErr
DFUDeviceInfoGet(tLMDFUDeviceState *pState, tDFUDeviceInfo *psDevInfo)
{
    ...
    sInfoCmd.ucCommand = TIVA_DFU_CMD_INFO;
...
//****************************************************************************
//
// Determine whether the connected device supports Tiva's DFU binary
// protocol and, if so, query the device information structure and fill in
// various fields in the state structure.
//
//****************************************************************************
static bool
CheckForTivaProtocol(tLMDFUDeviceState *pState)
{
    USHORT usCount;
    BOOL bRetcode;
    tLMDFUErr eRetcode;
    ...
    //
    // Clear out the Tiva-specific fields in the state structure.
    //
    pState->bSupportsTivaProtocol = false;
    ...
    bRetcode = Endpoint0Transfer(pState->hUSB, (REQUEST_TRANSFER_IN |
                          ...
                          USBD_DFU_REQUEST_TIVA,
                          REQUEST_TIVA_VALUE,
                          pState->usInterface,
                          sizeof(tLMDFUQueryTivaProtocol),
    ...
    //
    if(bRetcode && (usCount == sizeof(tLMDFUQueryTivaProtocol)) &&
       (sProt.usMarker == DFU_PROTOCOL_TIVA_MARKER) &&
       (sProt.usVersion == DFU_PROTOCOL_TIVA_VERSION_1))
    {
        //
        // We can safely use Tiva extensions when talking to this device.
        //
        pState->bSupportsTivaProtocol = true;
    ...
    //
    return(pState->bSupportsTivaProtocol);
}

//****************************************************************************
//
// Given the ulPartInfo value as returned in the DFU device information
// structure, return an integer representing the Tiva part number in
...
MapPartInfo(unsigned long ulPartInfo, char *strPart)
{
    ...
    //
    ucPartNo = (unsigned char)((ulPartInfo & TIVA_DFU_PART_M) >>
                               TIVA_DFU_PART_SHIFT);
    ...
    //
    for(iLoop = 0; iLoop < NUM_TIVA_PARTS; iLoop++)
    {
...
// determine the actual Tiva part number and revision we are talking to
...
GetPartNumberAndRev(tLMDFUDeviceState *pState, tLMDFUDeviceInfo *pDevInfo)
{
    ...
    // Tiva protocol or not.
    //
    pDevInfo->bSupportsTivaExtensions = pState->bSupportsTivaProtocol;

    //
    // Is the protocol supported?
    //
    if(pState->bSupportsTivaProtocol)
    {
        //
...
// should initiate a system reset.  Only devices supporting the Tiva
...
//
// \return Returns DFU_ERR_UNSUPPORTED if the device does not support Tiva
// protocol extensions.
//
//****************************************************************************
static tLMDFUErr
DFUDeviceReset(tLMDFUDeviceState *pState)
{
    ...
    if(!pState->bSupportsTivaProtocol)
    {
        TRACE("Device does not support Tiva protocol.\n");
        return(DFU_ERR_UNSUPPORTED);
    }
                ...
                tLMDFUHandle *phHandle)
{
    ...
    // call will only find devices identified by the Tiva DFU GUID.
    //
    hHandle = InitializeDeviceByIndex(DFU_VID, DFU_PID,
                                      (LPGUID)&(GUID_DEVINTERFACE_TIVA_DFU),
                                      iDeviceIndex, FALSE, &bInstalled);

    //
    // Did we find a device supporting the Tiva DFU class?
    //
    if(!hHandle)
    {
        //
    ...
    if(psDevInfo->bDFUMode)
    {
        ...
        //
        // Determine whether this device supports the Tiva-specific binary
        // protocol and, if so, read the device information structure.
        //
        CheckForTivaProtocol(pState);

        //
        // If possible, determine the specific Tiva part we are talking to.
        //
...
//****************************************************************************
//
//! Query DFU download-related parameters from a Tiva DFU device.
//!
...
//! a Tiva device.  These parameters include the device flash block size,
//! the number of flash blocks (hence the size of the flash), the programmable
//! address range, and the ID of the Tiva part itself.
//!
//! This function makes use of a Tiva-specific extension to the
//! DFU protocol so is only available on Tiva devices.
//!
//! \return Returns DFU_OK on success or other error codes on failure.
//
//****************************************************************************
...
//! \e pcDFUImage.
//! \param pbTivaFormat is a pointer which will be written to \e true if
//! the supplied data appears to start with a valid Tiva DFU prefix.
//! If the data ends in a valid DFU suffix structure but does not contain the
//! Tiva prefix, this value will be written to \e false.
//!
...
//! presence of a Tiva DFU prefix.  This structure contains the
//! address at which to flash the image and also the length of the payload.
//! The pbTivaFormat pointer is written to \e true if the following
...
//!    the suffix itself) and Tiva prefix (8 bytes).
//! 3. The unsigned short in bytes 2 and 3 of the image forms a sensible
//!    flash block number (address / 1024) for a Tiva device.
//!
...
LMDFUIsValidImage(tLMDFUHandle hHandle, unsigned char *pcDFUImage,
                   unsigned long ulImageLen, bool *pbTivaFormat)
{
    tLMDFUDeviceState *pState;
    unsigned long ulCRC;
    ...
    // device we intend using it with.  Does it have a Tiva prefix?  It
...
//! Tiva-specific header informing the device of the address to
...
//!
//! To flash a pure binary image without the DFU suffix or Tiva
//! prefix, use LMDFUDownloadBin() instead of this function.
//!
//! Note that the Tiva-specific prefix structure contains the address at
//! which the image is to be flashed so no parameter exists here to provide
//! this information.
//!
//! This function uses no Tiva-specific extensions to the DFU protocol and
...
//! Tiva extensions, the binary image will be downloaded to the
...
//! Tiva header to the device at an address supplied by the caller.
//!
//! This function is synchronous and will not return until the operation is
//! complete.  To receive periodic status updates during the operation, a
//! window handle may be provided.  This window will receive WM_LMDFU_STATUS
                 ...
                 bool bVerify, HWND hwndNotify)
{
    ...
    //
    // We only support binary downloads for Tiva targets since we
    // need to use a protocol extension to tell the board where to flash the
    // image.
    //
    if(!pState->bSupportsTivaProtocol)
    {
        TRACE("Target does not support Tiva DFU protocol.\n");
        return(DFU_ERR_UNSUPPORTED);
    }
    ...
    // image.
    //
    sProg.ucCommand = TIVA_DFU_CMD_PROG;
    sProg.ucReserved = 0;
    ...
    if((eRetcode == DFU_OK) && bVerify)
    {
        ...
        if((eRetcode == DFU_OK) && (sStatus.bState == STATE_IDLE))
        {
            ...
            sBin.ucCommand = TIVA_DFU_CMD_BIN;
            sBin.bBinary = 1;
            ...
            if(eRetcode == DFU_OK)
            {
                ...
                sRead.ucCommand = TIVA_DFU_CMD_READ;
           ...
           unsigned long ulEraseLen, bool bVerify, HWND hwndNotify)
{
    ...
    //
    sErase.ucCommand = TIVA_DFU_CMD_ERASE;
    sErase.ucReserved = 0;
    ...
    if((eRetcode == DFU_OK) && bVerify)
    {
        ...
        //
        sCheck.ucCommand = TIVA_DFU_CMD_CHECK;
        sCheck.ucReserved = 0;
...
//! Tiva-specific prefix and DFU standard suffix. If \e false, the wrappers
    ...
    if(eRetcode == DFU_OK)
    {
        ...
        if(eRetcode == DFU_OK)
        {
            ...
            if(eRetcode == DFU_OK)
            {
                ...
                // to the length of data we request via the TIVA_DFU_CMD_READ
